...
void call_TCP_server_throughput(void *args){
...
void call_TCP_server_latency(void *args){
    int server_fd, new_socket, valread;
    struct sockaddr_in address;
    int opt = 1;
    int addrlen = sizeof(address);
...
int main(int argc, char **argv)
{
        ...
        if(strcmp(connection_type,"TCP")==0){
        	if(block_size == 1){
        		create_threads_call_server(call_TCP_server_latency);
        	}else{
        		create_threads_call_server(call_TCP_server_throughput);
        	}
        }else{
